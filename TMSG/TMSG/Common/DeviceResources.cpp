...
void DX::DeviceResources::CreateDeviceResources() 
{
	...
	HRESULT hr = D3D11CreateDevice(
		...
		D3D_DRIVER_TYPE_HARDWARE,	// 하드웨어 그래픽 드라이버를 사용하여 장치를 만듭니다.
		...
		&m_d3dFeatureLevel,			// 만들어진 장치의 기능 수준을 반환합니다.
	...
	if (FAILED(hr))
	{
		...
		DX::ThrowIfFailed(
			D3D11CreateDevice(
				nullptr,
				D3D_DRIVER_TYPE_WARP, // 하드웨어 장치 대신 WARP 장치를 만듭니다.
				0,
				creationFlags,
				featureLevels,
	...
	// Direct3D 11.3 API 장치 및 직접 컨텍스트에 대한 포인터를 저장합니다.
	DX::ThrowIfFailed(
	...
	// Direct2D 장치 개체 및 해당 컨텍스트를 만듭니다.
...
void DX::DeviceResources::CreateWindowSizeDependentResources() 
{
	...
	else
	{
		...
		swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL;	// 모든 Windows 스토어 앱은 이 SwapEffect를 사용해야 합니다.
		swapChainDesc.Flags = 0;
		swapChainDesc.Scaling = scaling;
		swapChainDesc.AlphaMode = DXGI_ALPHA_MODE_IGNORE;

		// 이 시퀀스는 위에서 Direct3D 장치를 만드는 데 사용된 DXGI 팩터리를 가져옵니다.
		ComPtr<IDXGIDevice3> dxgiDevice;
		DX::ThrowIfFailed(
			m_d3dDevice.As(&dxgiDevice)
			);
		...
		// 응용 프로그램이 각 VSync 후에만 렌더링되어 전력 소비가 최소화됩니다.
		DX::ThrowIfFailed(
	...
	// 모든 Windows 스토어 앱에 회색조 텍스트 앤티앨리어싱을 사용하는 것이 좋습니다.
...
void DX::DeviceResources::ValidateDevice()
{
	...
	// 먼저, 장치를 만들었을 때의 기본 어댑터에 대한 정보를 가져옵니다.
...
// 모든 장치 리소스를 다시 만들고 현재 상태로 다시 설정합니다.
